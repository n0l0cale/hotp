...
public class OneTimePasswordAlgorithm {
	 ...
	 * @param codeDigits
	 *            the number of digits in the OTP, not including the checksum, if
	 *            any.
	 * @param addChecksum
	 *            a flag that indicates if a checksum digit should be appended to
	 *            the OTP.
	 * @param truncationOffset
	 *            the offset into the MAC result to begin truncation. If this value
	 *            is out of the range of 0 ... 15, then dynamic truncation will be
	 *            used. Dynamic truncation is when the last 4 bits of the last byte
	...
	static public String generateOTP(byte[] secret, long movingFactor, int codeDigits, boolean addChecksum,
		...
		String textstr = new String(text, StandardCharsets.UTF_8);
